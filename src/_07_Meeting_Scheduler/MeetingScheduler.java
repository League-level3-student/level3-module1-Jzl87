...
public class MeetingScheduler {
	...
	public static Schedule getMutualAvailability(Schedule person1, Schedule person2) {
		...
		HashMap<String, ArrayList<Integer>> map1 = person1.getSchedule();
		HashMap<String, ArrayList<Integer>> map2 = person2.getSchedule();

		for (String day : map1.keySet()) {
			for (Integer times : map1.get(day)) {
				for (Integer times2 : map2.get(day)) {
					if (times == times2) {
						result.addAvailability(day, times);
					}
				}
			}
		}
